...
export const useEmail = ({ onSuccess }: { onSuccess: () => void }) => {
    ...
    checkEmailMutation({ email }, {
      onSuccess: () => {
        sendCodeMutation({ email }, {
          onSuccess: () => { toast.success('인증번호가 전송되었습니다.'); setIsCodeSent(true); setTimer(180); },
          onError: (err: Error) => toast.error(err.message || '인증번호 전송 실패'),
        });
      },
      onError: (err) => {
  ...
  const handleVerifyCode = () => {
    verifyCodeMutation({ email, code: Number(authCode) }, {
      onSuccess: () => {
  ...
  useEffect(() => { if (timer === 0) setIsCodeSent(false); }, [timer]);
