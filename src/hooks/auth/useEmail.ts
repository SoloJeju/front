...
export const useEmail = ({
  ...
  const { mutate: checkEmailMutation, isPending: isCheckingEmail } =
    useMutation({ mutationFn: checkEmail });
  const { mutate: sendCodeMutation, isPending: isSendingCode } = useMutation({
    mutationFn: sendEmailCode,
  });
  const { mutate: verifyCodeMutation, isPending: isVerifyingCode } =
    useMutation({ mutationFn: checkEmailCode });
  ...
  const handleSendCode = () => {
    ...
    const sendCodeAction = () => {
      sendCodeMutation(email, {
        ...
        onError: (err: Error) => {
          console.error('인증번호 전송 API 에러:', err);
        ...
        { email },
        {
          ...
          onError: (err) => {
            if (isAxiosError(err) && err.response)
              toast.error(err.response.data.message || '이미 사용 중인 이메일');
  ...
  const handleVerifyCode = () => {
    verifyCodeMutation(
      { email, number: Number(authCode) },
      {
        ...
        onError: (err) => {
          if (isAxiosError(err) && err.response)
            toast.error(err.response.data.message || '인증번호가 틀립니다.');
