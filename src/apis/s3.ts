...
// S3 이미지 업로드
export const uploadImageToS3 = async (file: File): Promise<CommonResponse<{ imageName: string; imageUrl: string }>> => {
...
export const useImageUpload = () => {
  ...
  const removeImage = async () => {
    if (!uploadedImage) return { success: false, error: '삭제할 이미지가 없습니다.' };
