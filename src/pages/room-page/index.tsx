...
export default function RoomPage() {
        ...
        if (from === 'home') {
          ...
          if (response.isSuccess) {
            const room = response.result.find(
              (room: RecommendedChatRooms) => room.roomId === Number(roomId)
            );
            if (room) {
              setRoomData(room);
            } else {
  ...
  const room = {
    ...
    currentMembers:
      'currentParticipants' in roomData ? roomData.currentParticipants : 0,
    maxMembers: 'maxParticipants' in roomData ? roomData.maxParticipants : 0,
    scheduledDate:
      'scheduledDate' in roomData
        ? typeof roomData.scheduledDate === 'string'
          ? roomData.scheduledDate
          : roomData.scheduledDate.toISOString()
        : '',
